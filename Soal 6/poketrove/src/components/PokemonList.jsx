...
function PokemonList() {
  ...
  useEffect(() => {
    ...
    if (savedView) dispatch({ type: 'SET_VIEW_MODE', payload: savedView });

    const savedFilter = localStorage.getItem('filterType');
    if (savedFilter) dispatch({ type: 'FILTER_TYPE', payload: savedFilter });
  }, []);
        ...
        <div className="header">
          ...
          <div className="search-filters">
            <div className="filter-dropdown">
              <select value={state.filterType} onChange={(e) => handleFilterChange(e.target.value)}>
            ...
            <div className="view-buttons">
              <button className={state.viewMode === 'single' ? 'active' : ''} onClick={() => dispatch({ type: 'SET_VIEW_MODE', payload: 'single' })}>☰</button>
              <button className={state.viewMode === 'grid' ? 'active' : ''} onClick={() => dispatch({ type: 'SET_VIEW_MODE', payload: 'grid' })}>⊞</button>
            </div>
          </div>
        </div>
      </div>

      <div className="pokemon-grid-wrapper overflow-y-auto flex-grow px-4 pb-4">
